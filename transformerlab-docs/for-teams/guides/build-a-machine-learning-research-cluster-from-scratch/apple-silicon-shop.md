...
To use the GPU on Apple Silicon, the process must run **natively on macOS** (or strictly within a hypervisor that supports full GPU paravirtualization _and_ a matching driver, which is currently experimental/unstable for AI compute).
