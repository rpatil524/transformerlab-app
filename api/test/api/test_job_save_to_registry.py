...
def test_save_dataset_to_registry_copies_files(client, tmp_workspace):
    """Saving a dataset with target_name triggers the background copy to the registry."""
    job_id = "42"
    ...
    resp = client.post(
        ...
        params={"target_name": "my-dataset-group"},
    )
    assert resp.status_code == 200
...
def test_save_dataset_rejects_group_name_collision_with_flat(client, tmp_workspace):
    """mode='new' returns 409 when the group name matches a flat dataset with index.json."""
    job_id = "42"
    dataset_name = "my-dataset"
    group_name = "flat-dataset"
    _seed_job_dataset(tmp_workspace, job_id, dataset_name, content='{"row":1}')

    # Pre-create a flat dataset entry with an index.json at its root
    flat_dir = tmp_workspace["datasets_dir"] / group_name
    flat_dir.mkdir()
    (flat_dir / "index.json").write_text('{"format":"flat"}')

    resp = client.post(
        f"/experiment/alpha/jobs/{job_id}/datasets/{dataset_name}/save_to_registry",
        params={"target_name": group_name, "mode": "new"},
    )
...
def test_save_dataset_to_existing_registry_entry(client, tmp_workspace):
    ...
    # Pre-create the target group in the registry
...
def test_save_dataset_to_existing_requires_target_name(client, tmp_workspace):
    """mode='existing' without target_name falls back to source name which is not in registry."""
    job_id = "42"
    dataset_name = "my-dataset"
    ...
    # Falls back to source name as group; that group does not exist -> 404
    assert resp.status_code == 404
...
def test_save_model_to_registry_copies_files(client, tmp_workspace):
    """Saving a model with target_name triggers the background copy to the registry."""
    job_id = "42"
    model_name = "my-model"
    ...
    resp = client.post(
        ...
        params={"target_name": "my-model-group"},
    )
    assert resp.status_code == 200
    assert resp.json()["status"] == "started"
...
def test_save_model_rejects_group_name_collision_with_flat(client, tmp_workspace):
    """mode='new' returns 409 when the group name matches a flat model with index.json."""
    job_id = "42"
    model_name = "my-model"
    group_name = "flat-model"
    _seed_job_model(tmp_workspace, job_id, model_name, content="weights-v1")

    # Pre-create a flat model entry with an index.json at its root
    flat_dir = tmp_workspace["models_dir"] / group_name
    flat_dir.mkdir()
    (flat_dir / "index.json").write_text('{"format":"flat"}')
...
def test_save_dataset_and_model_from_same_job(client, tmp_workspace):
    ...
    # Save both to registry with target_name — both should start successfully
    ds_save = client.post(
        f"/experiment/alpha/jobs/{job_id}/datasets/{dataset_name}/save_to_registry",
        params={"target_name": "ds-group"},
    )
    ...
    model_save = client.post(
        f"/experiment/alpha/jobs/{job_id}/models/{model_name}/save_to_registry",
        params={"target_name": "model-group"},
    )
