...
def train_diffusion_lora():
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not load dataset {dataset_name}. Please check the dataset name and ensure it's available."
            )
        ...
        if eval_prompt:
            ...
            generate_sample_images(
                eval_pipeline, eval_prompt, 5, eval_images_dir, "before_train", args, device, weight_dtype
            )
        ...
        for epoch in range(num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % gradient_accumulation_steps == 0 or (step + 1) == len(train_dataloader):
                    ...
                    if global_step % 10 == 0:
                        lab.log(
                            f"Epoch {epoch + 1}/{num_train_epochs}, Step {global_step}/{max_train_steps}, Loss: {loss.item():.4f}"
                        )
            ...
            try:
                if not is_sdxl and not is_sd3 and not is_flux:
                    ...
                    save_file(
                        checkpoint_lora_state_dict, os.path.join(checkpoint_dir, "pytorch_lora_weights.safetensors")
                    )
        ...
        try:
            if not is_sdxl and not is_sd3 and not is_flux:
                ...
                save_file(
                    final_checkpoint_lora_state_dict,
                    os.path.join(final_checkpoint_dir, "pytorch_lora_weights.safetensors"),
                )
        ...
        if eval_prompt:
            ...
            generate_sample_images(
                eval_pipeline, eval_prompt, 5, eval_images_dir, "after_train", args, device, weight_dtype
            )
        ...
        if not is_sdxl and not is_sd3 and not is_flux:
            try:
                ...
                lab.log("✅ LoRA weights saved (SD 1.x)")
        ...
        if not saved_successfully and is_sdxl:
            try:
                ...
                lab.log("✅ LoRA weights saved (SDXL)")
                saved_successfully = True
            except Exception as e:
        ...
        if not saved_successfully and is_sd3:
            try:
                ...
                lab.log("✅ LoRA weights saved (SD3)")
                saved_successfully = True
            except Exception as e:
        ...
        if not saved_successfully and is_flux:
            try:
                ...
                lab.log("✅ LoRA weights saved (Flux)")
                saved_successfully = True
            except Exception as e:
