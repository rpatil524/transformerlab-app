...
# Training function
def train(
    model, device, train_loader, optimizer, epoch, log_interval, total_epochs=None, visualize=True, output_dir=None
):
    model.train()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if visualize and output_dir is not None:
            ...
            if (
                prediction_save_every_steps
                and prediction_save_every_steps > 0
                and (global_step % prediction_save_every_steps == 0)
            ):
                try:
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if idx < n:
                ...
                title = (
                    predictions[idx][0]
                    if hasattr(predictions[idx], "__len__") and len(predictions[idx]) > 0
...
def main():
    try:
        ...
        momentum = float(config.get("momentum", 0.5))
        ...
        if log_to_wandb:
            try:
                ...
                wandb.init(
                    project=os.environ.get("WANDB_PROJECT", "mnist-training-project"),
                    config=config,
                    name=f"{model_name}-{lab.job.id}" if hasattr(lab, "job") else model_name,
                    anonymous="allow",
                )
        ...
        if not (prediction_save_every_steps and prediction_save_every_steps > 0):
            ...
            lab.log(
                f"Skipping explicit 'before' snapshot since predictions will be saved at step_000 (every {prediction_save_every_steps} steps)"
            )
